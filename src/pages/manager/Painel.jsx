...
export default function Painel() {
  ...
  // 🧠 DEFINE O TURNO ATIVO (baseado no horário)
  const hora = new Date().getHours();

  const turnoAtual = hora < 15 ? "almoco" : "jantar";

  // 📊 METAS SEPARADAS POR TURNO
  const metaAlmoco = 500;
  const metaJantar = 300;

  const confirmadasTurno =
    turnoAtual === "almoco"
      ? confirmadas_almoco
      : confirmadas_jantar;
  ...
  const adesao =
    metaTurno > 0
      ? ((confirmadasTurno / metaTurno) * 100).toFixed(1)
      : 0;

  const semDados =
    !loading && confirmadasTurno === 0;

  const alertaJantar =
    confirmadas_jantar < confirmadas_almoco * 0.6;

  const adesaoJantar =
    confirmadas_almoco > 0
      ? ((confirmadas_jantar / confirmadas_almoco) * 100).toFixed(0)
      : 0;

  const taxaPresenca = 0.85;

  const recomendacaoAlmoco = Math.round(
    confirmadas_almoco / taxaPresenca
  );

  const recomendacaoJantar = Math.round(
    confirmadas_jantar / taxaPresenca
  );
  ...
  async function load() {
    ...
    const almoco = data?.filter(
      (d) => d.refeicao === "almoco"
    ).length || 0;

    const jantar = data?.filter(
      (d) => d.refeicao === "jantar"
    ).length || 0;
      ...
      <div className="space-y-4 pb-4">
        ...
        <div>
          ...
          <p className="text-sm text-gray-500">
            Turno atual:{" "}
            <strong>
              {turnoAtual === "almoco" ? "Almoço" : "Jantar"}
            </strong>
          </p>
        </div>
        ...
        {/* META VS REAL (DINÂMICO POR TURNO) */}
        <div className="bg-white rounded-2xl p-4 shadow-sm">

          <div className="flex items-center justify-between mb-4">
            <p className="text-xs font-semibold text-gray-500 uppercase">
              Meta vs Real ({turnoAtual})
            </p>
          </div>
          ...
          <div className="flex justify-between">
            ...
            <p className="text-xs text-gray-400">
              Faltam {metaTurno - confirmadasTurno}
            </p>
          </div>
        </div>
